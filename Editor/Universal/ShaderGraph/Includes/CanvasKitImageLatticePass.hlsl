...
Varyings BuildVaryings(Attributes input)
{
    ...
    positionOS.xy = ApplyImageLatticeDeformation(positionOS.xy, input.uv1);

    float3 positionWS = TransformObjectToWorld(positionOS);
    output.positionCS = TransformWorldToHClip(positionWS);

    #if UNITY_UV_STARTS_AT_TOP
    output.positionCS = TransformModelToHClip(positionOS);
    output.texCoord0.y = 1.0 - output.texCoord0.y;
        ...
        output.texCoord0 = input.uv0;
            ...
            output.texCoord1 = float4(positionOS.xy * 2 - clampedRect.xy - clampedRect.zw, 0.25 / (0.25 * half2(_UIMaskSoftnessX, _UIMaskSoftnessY) + abs(pixelSize.xy)));
        #endif
    #endif
...
half4 frag(PackedVaryings packedInput) : SV_TARGET
{
    ...
    color.a *= half(SAMPLE_TEXTURE2D(_AlphaTex, sampler_AlphaTex, unpacked.texCoord0.xy).r);
